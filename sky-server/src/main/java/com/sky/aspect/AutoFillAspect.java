...
public class AutoFillAspect {
    ...
    public void autoFill(JoinPoint joinPoint){
        log.info("开始公共字段自动填充");
        //1.使用joinPoint获取目标方法上的注解，拿到注解里的属性值
        ...
        AutoFill autoFill = method.getAnnotation(AutoFill.class); //获取指定的注解
        OperationType operationType= autoFill.value(); //获取注解中的属性值---操作类型

        //2.使用joinPoint获取到目标方法的参数对象
        Object[] args = joinPoint.getArgs(); //Object所有的都适用，不能强转
        if (args == null && args.length == 0) {
           return;
        }
        Object entity = args[0]; //拿到方法中传递的实体对象
        ...
        try {
            if(operationType == OperationType.INSERT){
                ...
                setCreatTime.invoke(entity,LocalDateTime.now());
                setUpdateTime.invoke(entity,LocalDateTime.now());
                setCreatUser.invoke(entity, BaseContext.getCurrentId());
